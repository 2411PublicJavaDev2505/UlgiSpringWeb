			...
			<div class="content">
				...
				<div class="pagination">
					<c:if test="${page.startNavi ne 1 }">
						<a href="/lease/search?currentPage=${page.startNavi-1 }&searchCondition=${search.searchCondition}&searchKeyword=${search.searchKeyword}">&lt;</a>
					</c:if>
					<c:forEach begin="${page.startNavi }" end="${page.endNavi }" var="i">
						<a href="/lease/search?currentPage=${i }&searchCondition=${search.searchCondition}&searchKeyword=${search.searchKeyword}" <c:if test="${page.currentPage == i }">style="color: #fff; background-color:#2d336a;"</c:if>>${i }</a>	
					</c:forEach>
					<c:if test="${page.endNavi ne page.maxPage }">
						<a href="/lease/search?currentPage=${page.endNavi+1 }&searchCondition=${search.searchCondition}&searchKeyword=${search.searchKeyword}">&gt;</a>
					</c:if>				
				</div>
			</div>
		</main>
